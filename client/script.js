...
const ohMy = () => {
    ...
    .then(res => {
        ...
        for(let i = 0; i < data; i++) {
          let newElements = i.createElement('p').textContent = data[i]
...
const repeatMyParam = () => {
    axios.get(`${baseURL}/repeat/'dog'`)
    .then(res => {
       let newText = document.getElementById('repeat-text')
       newText.textContent = res.data
    })
    .catch(error => {
        console.log(error)
    })
...
// CODE HERE
function getQuery () {
    axios.get(`${baseURL}/animals/?animals=Bears`)
    .then(res => {
